...
public class StringUtils
{
	...
	{		
		...
		{
			System.err.println("Bad Number {{"+str+"}}");
			return 0L;
		}
	}
	...
	public static String findTableWith(String html, String grep)
	{
		...
		System.err.println("Can't find table with "+grep+" in it!");
		return "";
	}
    ...
    public static String fromStringArray(Object[] arr, String delim)
    {
        StringBuffer ret = new StringBuffer();
        for (int i = 0; i < arr.length; i++)
        {
    ...
    public static String[] splitString(String str, String delim)
    {
        ...
        for (;;)
        {
            int o = str.indexOf(delim);
            if (o < 0)
                break;
            v.addElement(str.substring(0, o));
    ...
    public static String replace(String txt, String startsWith, String endsWith,
            String replacement)
    {
        int start = txt.indexOf(startsWith);
        if (start < 0)
            return txt;
        start += startsWith.length();
        int end = txt.indexOf(endsWith, start);
        if (end < 0)
            return txt;
    ...
    public static String escape(String text, char delim, String protect)
    {
        StringBuffer ret = new StringBuffer();
        for (char ch : text.toCharArray())
        {
            if (protect.indexOf(ch) >= 0)
                ret.append(delim);
            ret.append((char)ch);
        }
        return ret.toString();
    ...
    public static String unescape(String text, char delim)
    {
        StringBuffer ret = new StringBuffer();
        for (char ch : text.toCharArray())
        {
