...
public class DefaultChartStrategy implements IChartStrategy
{
    ...
    {
        DiffProfile diff = prof.getDifficulty();
...
//            used += addRandomNotes(midi, chart, ticksPerMeasure, quarter, diff, targetSteps);
        ...
        chart.setNotesDifficulty(prof.getName());
        chart.setNotesMeter(diff.getMeter());
    }

    private int addRandomNotes(MIDITune midi, SMChart chart,
    ...
    private int addNotes(List<MIDINote> selectedNotes, List<MIDINote> notesOfInterest, long granularity, int quota)
    {
        if (quota == 0)
            return 0;
        ...
        for (int k = 0; k < proj.getPatterns().size(); k++)
        {
            ...
            for (j = 0; j < pattern.getInstances().size(); j++)
            {
                ...
                for (int i = 0; i < selectedNotes.size(); i++)
                {
                    ...
                    {
                        used++;
                        System.out.print(patternToStep(step));
                        beats.add(beat);
                    }
                }
                System.out.println();
    ...
    private String patternToStep(String pattern)
    {
    ...
    private boolean intersects(long startTick, long endTick,
    ...
    private String randomNote(DiffProfile diff)
    {
        int roll;
    ...
    private SMBeat setNote(SMChart chart, long ticksPerMeasure, MIDINote note, Long tick, String step)
    {
        if (step.equals("0000"))
            return null;
        if (tick == null)
    ...
    private void expandMeasure(SMMeasure m, long newGap)
    {
    ...
    private List<MIDINote> findNotes(MIDITune midi, long startTick, long endTick)
    {
    ...
    private void annotateBeats(SMChart chart, long ticksPerMeasure, MIDITune midi, 
...
//        float minuteElapsed = minuteEnd - minuteStart;
    ...
    private void annotateDoubles(DiffProfile diff, List<SMBeat> beats)
    {
    ...
    private void annotateHolds(SMChart chart,
    ...
    private void annotateMines(long ticksPerMeasure, DiffProfile diff, List<SMBeat> beats)
    {
        final Map<SMBeat, Long> suitability = new HashMap<>();
        long q = ticksPerMeasure/4;
        for (int i = 0; i < beats.size(); i++)
    ...
    private void removeChoices(Set<Integer> choices, char[] notes)
    {
