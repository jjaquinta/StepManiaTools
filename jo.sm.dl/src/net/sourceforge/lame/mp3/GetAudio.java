...
public class GetAudio {
  ...
  private DataInputStream musicin;
  ...
  int read_samples_mp3(final LameGlobalFlags gfp, DataInputStream musicin,
                       float mpg123pcm[][]) {
    int out;
  ...
  private int unpack_read_samples(final int samples_to_read,
                                  ...
                                  final int[] sample_buffer, final DataInputStream pcm_in)
      throws IOException {
   ...
   */
  private int read_samples_pcm(final DataInputStream musicin,
      ...
      if (type == IFF_ID_COMM) {
        ...
        try {
          //pcm_data_pos = sf.getFilePointer();
            throw new IOException("get file pointer not supported");
        } catch (IOException e) {
          return -1;
        }
        /*
  ...
  private DataInputStream OpenSndFile(final LameGlobalFlags gfp,
                                       final String inPath, final FrameSkip enc) {
    ...
    try {
      if (gfp.mInputStream != null)
          musicin = new DataInputStream(gfp.mInputStream);
      else
          musicin = new DataInputStream(new FileInputStream(inPath));
  ...
  private int lame_decode_initfile(final DataInputStream fd,
   ...
   * on MP3 file.
   */
  private int lame_decode_fromfile(final DataInputStream fd,
  ...
  private int Read32BitsLowHigh(final DataInputStream fp) {
  ...
  private int Read16BitsLowHigh(final DataInputStream fp) {
    try {
  ...
  private int Read16BitsHighLow(final DataInputStream fp) {
    try {
  ...
  private int Read32BitsHighLow(final DataInputStream fp) {
  ...
  private double readIeeeExtendedHighLow(final DataInputStream fp)
      throws IOException {
